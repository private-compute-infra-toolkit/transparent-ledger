...
# Starts or stops the TLedger enclave on the system, notifying systemd of status
...
#
# Usage:
#   tledger_watcher.sh (start|stop)
set -euo pipefail

# Allow values to be overridden with environment variables.
ENCLAVE_PATH="${ENCLAVE_PATH:-/opt/google/tledger/enclave.eif}"
...
ENABLE_TLEDGER_DEBUG_MODE="${ENABLE_WORKER_DEBUG_MODE:-0}"
declare -a TLEDGER_DEBUG_MODE_FLAGS=()

if [[ $ENABLE_TLEDGER_DEBUG_MODE != 0 ]]; then
  TLEDGER_DEBUG_MODE_FLAGS=(--debug-mode)
fi
...
start_enclave() {
  ...
  if ! nitro-cli run-enclave --cpu-count="${cpu}" --memory="${memory}" --eif-path "${ENCLAVE_PATH}" "${TLEDGER_DEBUG_MODE_FLAGS[@]}"; then
    handle_enclave_startup_failure
    sleep 10
    systemd-notify ERRNO=1
    exit 1
