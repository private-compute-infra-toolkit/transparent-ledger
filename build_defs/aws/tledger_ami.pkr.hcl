...
locals {
  ...
  # List of additional systemd configuration to add to tledger.
  tledger_overrides = [
...
build {
  ...
  provisioner "file" {
    content = <<-EOF
    [Service]
    ${join("\n", local.tledger_overrides)}
    EOF
    destination = "/tmp/tledger_override.conf"
  }

  # Create worker overrides directory and move override there;
  provisioner "shell" {
    inline = [
      "sudo mkdir /etc/systemd/system/tledger.service.d/",
      "sudo mv /tmp/tledger_override.conf /etc/systemd/system/tledger.service.d/override.conf",
